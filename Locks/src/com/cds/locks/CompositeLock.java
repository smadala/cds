...
public class CompositeLock implements Lock{
	...
	private static final int MIN_BACKOFF=1; //in milliseconds
	private static final int MAX_BACKOFF=30;
	...
	public CompositeLock(){
		tail= new AtomicStampedReference<CompositeLock.QNode>(null, 0);
	...
	private QNode acquireQNode(Backoff backoff, long startTime, 
			long patience) throws TimeoutException, InterruptedException{
	...
	private QNode spliceQNode(QNode node, long startTime, 
	...
	private void waitForPredecessor(QNode pred, QNode node, 
			long startTime, long patience) throws TimeoutException{
		int [] stamp={0};
		if(pred == null){
			myNode.set(node);
			return;
